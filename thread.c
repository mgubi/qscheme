...
void scm_thread_write(SOBJ x, PORT *p)
{
  ...
  scm_write_obj(scm_asprintf("#<thread id=%d flags=%d>",
							 v->tid, v->tflags),
				p,1);
}
...
SOBJ scm_thread_dump(SOBJ x)
{
  ...
  v = SCM_THREAD(x);
  scm_cdisplay(scm_asprintf("#<thread id=%d flag=%02x code=%d "
...
{
  scm_write_obj(scm_asprintf("#<mutex %p>", SCM_MUTEX(x)), p, 1);
}
...
{
  scm_write_obj(scm_asprintf("#<semaphore %p>", SCM_SEMAPHORE(x)), p, 1);
}
