...
char *scm_number2cstr(SOBJ n, long base)
{
  char *s;
  char buf[128];
  ...
  case SOBJ_T_FNUM:
	...
	sprintf(buf, "%.15g", SCM_FNUM(n));
	return(scm_must_strdup(buf));
...
SOBJ scm_number_to_string(SOBJ x, SOBJ y)
{
  ...
  p = scm_number2cstr(x, base);
  x = scm_newcell(SOBJ_T_STRING);
  SCM_STR_LEN(x) = strlen(p);
  SCM_STR_VALUE(x) = p;
  return(x);
}
